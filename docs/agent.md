...
flowchart TD
    ...
    C --> D[Computar SHA-256 para cada archivo]
    D --> E[Clasificar cada archivo vs estado API<br/>usando la tabla §2.5]
E --> F[Agrupar en batch]
F --> G[POST /api/sources/reconcile]
G --> H[API procesa y persiste]
```
...
4. Agent computa SHA-256 para cada archivo del FS con timeout configurable (§2.6). Los archivos
   que fallen el hash (timeout, max-size, write-race) se excluyen de la clasificación y se
   reintentan en el próximo escaneo.
5. Agent clasifica cada archivo contra el estado conocido usando su content hash (tabla §2.5).
...
| 5 | Path excede el límite de longitud del SO                | El Agent usa exclusivamente NIO (Java 21), que soporta paths largos nativamente sin `\\?\`. No se requiere acción. Ver `docs/issues/ISSUE-03-LongPath.md`.           |
...
| #  | Caso                                                | Comportamiento                                                                                                                                                                                                                                                                                                                                                                                                  |
|----|-----------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 26 | Path duplicado en la respuesta de la API            | **No aplica.** La API garantiza `pathLower` único por fila en `GET /api/sources/paths` (`api.md` §2.1): los activos preceden a los orphans y los duplicados se filtran. Si aún así ocurriera (error de implementación), ignorar el duplicado, usar el primero y log WARN.                                                                                                                                       |
...
| 30 | Archivo cambia entre GET /paths y POST /reconcile   | Ventana pequeña (minutos). La API procesa operaciones duplicadas como no-op. Si un CREATE falla porque el path ya no existe, la API lo maneja internamente. Si un `sourceId` no existe en RENAME/UPDATE/DELETE/REACTIVATE (fue purgado por el usuario entre GET y POST), la API responde `SOURCE_NOT_FOUND` en `errors`. El Agent loguea WARN y continúa — no reintenta porque la operación ya no es relevante. |
...
| 35 | Dos reconciliaciones superpuestas                                                                                                | Usar `AtomicBoolean` como lock entre reconciliaciones. La reconciliación manual se detecta por polling (`GET /api/reconcile/pending`) y no utiliza cola local — el flag `pending` en la API actúa como cola de máximo 1. Si el poll detecta `pending = true` pero hay una reconciliación en curso, se omite (log DEBUG); el próximo poll lo reintentará. Cuando el Agent inicia efectivamente la reconciliación manual (adquiere el lock), llama a `POST /api/reconcile/ack` para resetear el flag. Si el lock está ocupado, omite el ack y reintenta en el próximo ciclo del poller. Si el Agent crashea después del ack pero antes de comenzar el escaneo, el próximo escaneo programado (cada 5 min) lo recupera.                                                                                                                                                                                         |
...
| 43 | **Race entre POST /api/reconcile del frontend y ack del Agent**                                                                  | El frontend solicita reconciliación (pending=true) justo antes de que el Agent llame a ack (pending=false). La API responde 200 (idempotente) pero el ack resetea el flag inmediatamente después — la reconciliación solicitada nunca ocurre. Impacto bajo: el escaneo en curso cubre los cambios. El usuario puede reintentar.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 44 | **Pending flag stuck por crash del Agent antes del ack**                                                                         | Si el Agent crashea después de que el frontend setea `pending=true` pero antes de llamar a `POST /api/reconcile/ack`, el flag queda `true` en la API permanentemente. Al reiniciar, `Agent.main()` llama a `POST /api/reconcile/ack` como parte de la inicialización (antes de arrancar los executors), limpiando cualquier flag huérfano. Si no hay flag pendiente, el ack es no-op.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
cambiado, el Agent clasificaría como RENAME (caso D) y enviaría `pathLower` junto con el nuevo path.
...
```mermaid
flowchart LR
    A[Scanner<br/>Files.walkFileTree] -->|List<NormalizedPath>| B[Hasher<br/>SHA-256]
    B -->|List<NormalizedPath><br/>con hash| C[Classifier<br/>tabla §2.5]
...
- Recibe la lista de paths del FS **con content hash ya computado** y el estado conocido de la API
  (`GET /api/sources/paths`).
- Construye índices en memoria: `Map<pathLower, SourceState>` y `Map<contentHash, SourceState>`.
- Clasifica cada archivo contra el estado conocido aplicando la tabla de §2.5. Como todos los
  archivos tienen hash, puede determinar el tipo de operación sin necesidad de hashing diferido.
- Los archivos que fallaron el hashing (timeout, max-size, write-race) se excluyen y no generan
  operación. Se reintentarán en el próximo escaneo.
...
- Recibe la lista completa de paths del FS (`List<NormalizedPath>` desde Scanner).
- Computa SHA-256 para **cada** archivo con `MessageDigest.getInstance("SHA-256")` + `DigestInputStream`.
  No hay distinción por "casos que requieren hash" — el Classifier usará los hashes para determinar
  el tipo de operación.
- Timeout configurable por archivo (default: 30s).
- Write-race detection: verifica `Files.size()` antes y después del cómputo. Si el tamaño cambió,
  descarta el hash y marca el archivo como fallido (se reintenta en el próximo escaneo).
- **Errores:** timeout → skip con log WARN, archivo > tamaño máximo configurado → skip con log WARN,
  sin permisos de lectura → skip con log WARN, write-race → skip con log WARN (contador de reintentos).
- **Produce:** `List<NormalizedPath>` enriquecida con `contentHash` (String hexadecimal de 64 chars)
  para archivos hasheados exitosamente. Archivos fallidos se marcan para exclusión del Classifier.
...
sequenceDiagram
    ...
    Recon ->> Recon: Files.walkFileTree + hash + batch
...
- **Cleanup de pending flag:** antes de arrancar los executors, `Agent.main()` llama a `POST /api/reconcile/ack` para
  limpiar cualquier flag `pending` que haya quedado `true` por un crash anterior (EC44). Si no hay flag pendiente,
  el ack es no-op.
